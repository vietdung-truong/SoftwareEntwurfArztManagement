...
public class JDBCCreateDatabase {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * //Here connecting System.out.println("Getting connection..."); 
		 * conn = DriverManager.getConnection(DB_URL, Username, Password);
		 * System.out.println("Connection established");
		 * 
		 * //Creating Statement and Database Here:
		 * System.out.println("Establishing Statement..."); 
		 * stmt = conn.createStatement(); System.out.println("Statement established");
		 ...
		 * String sql = String.format("CREATE DATABASE %s", s); stmt.executeUpdate(sql);
		 * System.out.println("Database Has been created");
		 * 
		 * 
		 * } catch (ClassNotFoundException e) { // TODO Auto-generated catch block
		 * e.printStackTrace();
		 * } catch (SQLException se) { // TODO Auto-generated catch
		 * block se.printStackTrace(); 
		 * }finally { if (conn != null) { //sobald alle
		 * Befehle abgeschlossen sind/ eine connection geöffnet wurde und bearbeitet
		 * wurde, schließt sich die Verbindung 
		 * try { conn.close(); 
		 * } catch (SQLException
